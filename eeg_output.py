...
# global variables added by JZ 11/18/24
RECORD_FILE_NAME = "bci_input_data/eegdata_1.csv"
RECORD_FILE      = "bci_input_data/eegdata_"
OUTPUT_FILE      = "bci_data_transit/eegdata_"
OUTPUT_FILE_LINES = 0
label = 1
                    ...
                    if self.KeyModel == 2 or self.KeyModel == 1:
                        ...
                        if self.format < 1:
                            ...
                            if cyIO.isRecording() == True:
                                ...
                                cyIO.startRecord(str(datetime.datetime.now()) + (counter_data + packet_data))   # record with time_stamp
                            if self.outputdata == True:
                                #mirror.text(str(datetime.datetime.now()) + str(counter_data + packet_data))
                                f = open(RECORD_FILE_NAME, "a")
                                f.write(str(packet_data) + "," + str(label) + "\n")
                                f.close()
                                OUTPUT_FILE_LINES += 1
                                if OUTPUT_FILE_LINES >= (128 * 1):
                                    os.rename(RECORD_FILE_NAME, OUTPUT_FILE + str(label) + ".csv")
                                    label += 1
                                    RECORD_FILE_NAME = RECORD_FILE + str(label) + ".csv"
                                    OUTPUT_FILE_LINES = 0

#                                #f = open("eegdata.csv", "a")   # another method of recording with timestamp
#                                #f.write(str(datetime.datetime.now()) + str(counter_data + packet_data))
#                                #f.close()
#
#                        
#                        
